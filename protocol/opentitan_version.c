...
int libhoth_extract_ot_bundle(const uint8_t* image, size_t image_size,
  ...
  if ((offset+OPENTITAN_OFFSET_APP_FW+OPENTITAN_OFFSET_VERSION_MINOR+4) > image_size) {
    fprintf(stderr, "Image offset is invalid");
    return -1;
  }
  // Checks that the image offset won't overflow the offset calculations 
  if ((offset+OPENTITAN_OFFSET_APP_FW+OPENTITAN_OFFSET_VERSION_MINOR+4) < offset) {
